...
metadata:
  ...
  raise.outputs: '- mr_url: string, git
    ...
    - retro_md: file_path (work/bugs/{issue_key}/retro.md)
...
|:-----:|----------|----------------------|
| 6 | `work/bugs/{issue_key}/retro.md` | **close** |
| 5 | Code commits on bug branch after `plan.md` | **review** |
| 4 | `work/bugs/{issue_key}/plan.md` | **fix** |
| 3 | `work/bugs/{issue_key}/analysis.md` | **plan** |
| 2 | `work/bugs/{issue_key}/scope.md` with `^TRIAGE:` | **analyse** |
| 1 | `work/bugs/{issue_key}/scope.md` without TRIAGE | **triage** |
| 0 | (nothing) | **start** |
...
1. Check Jira comments for resolution evidence: `rai backlog get-comments {issue_key} -a jira`
2. Search release branch for fix commits: `git log release/{version} --grep="{issue_key}" --oneline`
3. If evidence of prior fix found, present to user:

```
── Already-Resolved Check ──

{issue_key} may already be fixed:
...
- Use `raise_backlog_transition` MCP tool with issue_key="{issue_key}" and status="In Progress"

Query the graph for relevant context (single query, non-blocking):
- Use `raise_graph_query` MCP tool with query="bugs patterns {component or keywords}" (limit 5)

If MCP tools are not available, fall back to:
```bash
rai backlog update {issue_key} --assignee "{developer-email}" -a jira
rai backlog transition {issue_key} in-progress -a jira
...
After all phases complete, use `raise_backlog_transition` MCP tool with issue_key="{issue_key}" and status="Done".

If MCP tools are not available, fall back to:
```bash
rai backlog transition {issue_key} done -a jira
```

### Step 5: Complete & Report

```markdown
## Bugfix Run Complete: {issue_key}
...
| Phase | File | Op |
|-------|------|:--:|
| start | `work/bugs/{issue_key}/scope.md` | + |
| triage | `work/bugs/{issue_key}/scope.md` | ~ |
| analyse | `work/bugs/{issue_key}/analysis.md` | + |
| plan | `work/bugs/{issue_key}/plan.md` | + |
| fix | `src/path/to/file.py` | ~ |
| review | `work/bugs/{issue_key}/retro.md` | + |
...
| Item | Destination |
|------|-------------|
| All bug artifacts | `work/bugs/{issue_key}/` |
